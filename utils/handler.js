...
async function registerCommands(client, dir = "" /* If there is no directory specified it is equal to:' '*/) { // Create the function
    ...
    for (const file of files) { // For a file in the files
        const stat = await fs.lstat(path.join(filePath, file)); // Get the data from the file
        if (stat.isDirectory()) { // If the file is a dir
        ...
        } else if (file.endsWith(".js")) { // If the file is a js file
            const Command = require(path.join(filePath, file)); // Load the file data
            if (Command.prototype instanceof BaseCommand) { // If the file is an instance of our base command file
...
async function registerEvents(client, dir = "" /* If there is no directory specified it is equal to:' '*/) { // Create the function
    const filePath = path.join(__dirname, dir); // Get the file path
    const files = await fs.readdir(filePath); // Read all the files in the path directory
    for (const file of files) { // For a file in the files
        const stat = await fs.lstat(path.join(filePath, file)); // Get the data from the file
        if (stat.isDirectory()) { // If the file is a dir
            ...
            if (Event.prototype instanceof BaseEvent) { // If the file is an instance of our base event file
                const event = new Event(); // Create a new event
                console.log(
                    "🎉  Successfully registered " + event.name + " event !"
                );
